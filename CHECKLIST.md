...
- [x] **Sblocco immediato heca**: impostato `plannerAutoEnable: true` + `plannerAutoStartWeb: true` in `heca/.planner/project.json` → entrambi i prompt saltati (auto-enable + auto-start-web) → nessuna `ctx.ui.input()` in `session_start` → Pi si sblocca con planner **abilitato**.

### Prossimi passi
- [ ] Web UI: pagina requirements (vedi `BACKLOG.md` P2-3)
- [ ] Manutenere README + guida d'uso in sync (vedi `BACKLOG.md` P2-4)
- [ ] **Republish NPM 0.2.7**: `pnpm build:pi-adapter && pnpm --filter @agent-plan/pi-adapter publish --access public --no-git-checks` (include il safety net del prompt). Server 0.2.4 + core 0.2.4 già pubblicati.
- [ ] Indagare perché `ctx.ui.input()` non renderizza quando l'estensione è caricata da NPM (possibile bug Pi con prompt ESM in `session_start` / timing di inizializzazione UI). Il timeout è un workaround; la causa andrebbe aperta come issue Pi.
