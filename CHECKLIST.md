...
- [x] **F002 — Tool web lifecycle esposti all'agente (pi-adapter 0.2.14)**: registrati 3 agent tool (`planner-web`, `planner-load`, `planner-stop`) in `packages/pi-adapter/src/index.ts` che wrappano le funzioni interne degli slash command. Parità con `@agent-plan/mcp`. Motivo: i comandi `/planner *` non sono intercettati con planner disabilitato; ora l'agente Pi gestisce il web via tool. PR #17 (branch separato `feat/pi-adapter-web-agent-tools`).
