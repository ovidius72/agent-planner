...
- [ ] Handoff page: accordion chiuso di default per ogni handoff elencato nella route `/handoff`
- [ ] Web UI polish: dashboard controls, handoff icon, stepper cleanup
...
### Fatto — F6 EHPKV: Handoff resilience + Web UI animation gate (sessione 2026-07-27)
- [x] **Problema**: F004 usava "delete-on-resume" (l'agente cancellava l'handoff appena letto) → fragile, perdi contesto se riprendi più tardi. Inoltre il path file-based legacy (`HANDOFF.md`) era deprecato ma non rimosso (metodi morti in plan-store.ts). E l'animazione `ap-in-progress` si attivava su feature/fasi "in-progress" anche senza task attivi.
- [x] **Fase 1 — Animation gate** (phase f56b3f03, T001): `packages/plan-web-ui/src/components/dashboard/work-tree-rows.tsx` — feature row (L99) + phase row (L201) ora gateano `ap-in-progress` su `hasActiveTask` (non su `status==="in-progress"`); se status in-progress ma nessun task attivo → border/bg sottile. Task row invariato (gates su `task.status`).
- [x] **Fase 2 — Handoff resilience** (phase 1e6b3ec9, 4 task):
  - **GLBDM** — Schema (`packages/plan-core/src/schema.ts`): aggiunti `handoffReadAt` (ISO, letto su recap) + `handoffHistory` (array di `HandoffHistoryEntrySchema`: {file, clearedAt, reason}, cap 5) a `PhaseSchema`. `HandoffHistoryEntrySchema` dichiarato prima di `PhaseSchema` (use-before-declaration).
  - **RPKH3** — PlanStore (`packages/plan-core/src/plan-store.ts`): `handoffArchiveDir()` → `.planner/handoff-archive/`; `markHandoffRead(phaseId)` (set handoffReadAt, keep content); `clearPhaseHandoff(phaseId, reason)` → archivia markdown in `handoff-archive/<phaseId>-<ISO>.md`, prepende entry in `handoffHistory` (cap 5, oldest file deleted), wipe handoff. No-op se vuoto. `syncTaskStatusRollup` → `clearPhaseHandoff(phaseId, "phase-done")`. `.gitignore`: aggiunto `handoff-archive/`.
  - **F92HK** — Adapters: pi-adapter `session_start` → `markHandoffRead` per ogni handoff pendente (NO clear-on-read); `task_start` → auto-archive handoff della fase del task (reason "task-started") + notify informativa per altre fasi; `handoff clear`/`plan_delete_handoff`/`handoff_clear` → reason "manual"; protocollo #1 HANDOFF HYGIENE riscritto (keep until task starts, no manual clear); protocollo #6 ENTITY REFERENCES → shortId PRIMARY (T00x ambiguo senza full qualifier). plan-mcp `task_start` → auto-archive dopo `found`; `handoff_clear` → reason "manual". plan-server + plan-mcp Phase literals: aggiunti `handoffReadAt`+`handoffHistory`. plan-web-ui `types.ts`: `HandoffHistoryEntry` interface + campi Phase.
  - **GDSHS** — Rimozione legacy file path: rimossi `handoffPath()`/`handoffExists()`/`loadHandoff()`/`saveHandoff()`/`deleteHandoff()` (dead code, zero caller) da plan-store.ts. Aggiunto `importLegacyHandoffFile()` (import one-time idempotente: se `HANDOFF.md` esiste + non vuoto + fase target senza handoff → scrive su fase in-progress + rename a `.bak`; entity-scoped vince se fase ha già handoff). Chiamato da `ensureStructureOrdering()`. Deprecated tool aliases `plan_*_handoff` kept (entity-scoped redirects).
- [x] **Repair**: feature `be763011` (F6 EHPKV) era orphaned (file mancante, nessun .bak) — ricreata con stesso id + 2 phaseIds. `plan repair`: 0 dangling.
- [x] Validazione: `pnpm -r build` + `pnpm check` puliti. Test funzionale import legacy su copia temp: imported=true → P001(F001), HANDOFF.md → .bak, seconda run no-op.
- [ ] **Da fare al release**: bump `@agent-plan/core` (schema+planstore), `@agent-plan/pi-adapter` (adapters), `@agent-plan/server` (literal), `@agent-plan/mcp` (adapter+literal), `@agent-plan/web-ui` (types). Branch attuale `next`.

### Fatto — Slim phase/feature/project tool responses + derived-status note (sessione 2026-07-27)
- [x] **Problema (segnalato dall'agente heca)**: `planner-phase-update` in Claude Code (plan-mcp) passava l'oggetto fase **intero** come `structuredContent`. Quando la risposta era troppo grande per il display, l'agente la scambiava per un fallimento — anche se `writeGenerated()` aveva già applicato la modifica. Stessa classe di bug del task-response slimming fatto in precedenza (phase/feature create/update erano stati lasciati pieni: "non fixati ora").
- [x] **Fix plan-mcp** (`packages/plan-mcp/src/index.ts`): importato `formatFeatureRef`; slimmati 8 caller `writeAndSummarize` (project-init, language-prefs, project-discuss, feature-create/update, phase-create/discuss/update) — droppato `structuredContent`, messaggio slim con compositeRef (`F00x`/`P00x(F00x)`) + shortId, no UUID. Refactor dei phase-discuss/update per caricare `features` in una var (serve per `featureNumberOfPhase`). Phase/feature delete lasciati small. Task tools erano già slim.
- [x] **Fix pi-adapter** (`packages/pi-adapter/src/index.ts`): slimmati 4 return LLM-facing (`content.text`): feature-create/update (`formatFeatureRef`+shortId), phase-create/update (`formatPhaseRef`+`featureNumberOfPhase`+shortId, con load features). `ctx.ui.notify` lasciati (Pi-internal, non LLM-facing). Project-update già slim.
- [x] **AGENTS.md**: aggiunta nota "Stato derivato, non persistito" sotto la regola 2 — lo status di feature/fase in `.planner/*.json` è derivato (può essere `null`/stale); la fonte di verità è la risposta del tool, non il JSON grezzo. Un agente che legge `status:null` e lo scambia per "non fatto" sbaglia.
- [x] Validazione: `pnpm -r build` + `pnpm check` puliti. Smoke-test MCP stdio `planner-phase-update`: `✅ Phase updated: P005(F004) — <title> · 8LTJ6`, `structuredContent=false`, no UUID, compositeRef presente.
- [x] ** anche **: riparato link rotto F1→phase (F1.phaseIds era `[]` mentre phase 5714df9d aveva featureId=F1); chiuso task stale MN7GL (b198dbb8, "Derived phase/feature status" — era tracking artifact, il rollup è implementato).
- [ ] **Da fare al release**: bump `@agent-plan/mcp` + `@agent-plan/pi-adapter` (rebuild dist). Branch `next`.

### Fatto — Recap: handoff authoritative over resume.json nextSteps (staleness fix) (sessione 2026-07-27)
- [x] **Problema (segnalato dall'agente heca)**: `refreshResume` (plan-store.ts L751) keepa i `nextSteps`/`notes` free-text per sempre (`existing?.nextSteps ?? []`) — vanno stale silenziosamente, ma il recap li presentava come "Next step" autorevole, fuorviando la ripresa. heca: "nextSteps still read 'T009 ... do not start it' after the user had decided Level 2 the day before". F6 ha reso l'handoff (`phase.handoff`) il contesto resume autorevole + gestito attivamente — ma il recap leggeva ANCHE i nextSteps stale di resume.json: due fonti in competizione, quella stale poteva sovrascrivere l'handoff fresco.
- [x] **Fix schema** (`packages/plan-core/src/schema.ts`): aggiunto `nextStepsUpdatedAt` a `ResumeFocusSchema` (quando i nextSteps sono cambiati l'ultima volta).
- [x] **Fix PlanStore** (`packages/plan-core/src/plan-store.ts`): `saveResume` traccia `nextStepsUpdatedAt` — bumpato solo quando saveResume riceve nextSteps nuovi (confronto JSON), preservato attraverso `refreshResume` (che keepa i nextSteps esistenti). Aggiunto il campo ai 3 letterali ResumeFocus (init, guardBypass fallback, refreshResume).
- [x] **Fix buildRecap** (`packages/plan-core/src/recap.ts`, condiviso Pi + MCP/Claude Code/Codex):
  - Con ≥1 handoff pendente: "Next step" punta all'handoff (autorevole, gestito attivamente); i nextSteps legacy di resume.json sono **soppressi** con nota.
  - Senza handoff: fallback su `resume.nextSteps[0]` + caveat ⚠️ ("free-text from resume.json, may be stale — verify against current state") + la data `nextStepsUpdatedAt`.
  - Aggiornata la direttiva handoff: "delete-on-resume" → "KEPT until a task in that phase starts (auto-archived) or the phase completes — no need to clear manually" (semantica F6). Rimosso `handoffClearCmd` morto.
