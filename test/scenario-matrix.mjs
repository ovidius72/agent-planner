...
export const scenarios = [
  ...
  },
  {
    ...
    harnesses: ['mcp', 'pi'],
    fixture: "minimal",
    surfaces: ['mcp.handoffPrepare', 'mcp.handoffWrite', 'mcp.handoffShow', 'mcp.handoffVerify', 'pi.tool.handoffWrite', 'pi.tool.handoffShow', 'pi.tool.handoffVerify'],
    steps: [
      "Run handoff-prepare.",
      ...
      "After confirmation, write on that exact phaseRef and assert the result is persisted but resumeReady=false.",
      "Show and read the complete persisted body; verify that reporting omissions blocks resume readiness.",
      "Repeat the five source reviews with the shown content hash and no omissions; assert handoff-verify returns resumeReady=true.",
    ],
    expects: { ok: true },
  },
  {
