...
import { cleanupFixtures, readPlanSnapshot } from "./helpers/fixtures.mjs";
import { startServerFixture, closeServerFixture, request as serverRequest } from "./helpers/server-fixture.mjs";
import { startMcpFixture, closeMcpFixture, cleanupMcpFixtures, callTool } from "./helpers/mcp-fixture.mjs";
import { createPiHost, closePiHost, cleanupPiHosts } from "../packages/pi-adapter/test/helpers/pi-host-fixture.mjs";
import { runScenario, registerExecutor, normalizeCoreResult, normalizeHttpResult, normalizeToolResult } from "./helpers/runner.mjs";
...
function assertParity(results, harnesses, label, { compareSnapshot = true } = {}) {
  const baseline = results[harnesses[0]];
  for (const harness of harnesses.slice(1)) {
    const current = results[harness];
    assert.equal(current.ok, baseline.ok, `${label}: ok mismatch for ${harness}`);
    assert.equal(current.errorCategory, baseline.errorCategory, `${label}: errorCategory mismatch for ${harness}`);
    if (compareSnapshot) {
      assert.deepEqual(comparableSnapshot(current.snapshot), comparableSnapshot(baseline.snapshot), `${label}: normalized snapshot mismatch for ${harness}`);
    }
  }
}
...
for (const entry of parityCases) {
  test(`T254 parity: ${entry.id}`, async () => {
    ...
    assertParity(results, entry.harnesses, entry.id, entry);
  });
}
