...
3. **Before generating prose**, copy the exact returned `draftTemplate`. Replace every `{{REQUIRED: ...}}` placeholder and retain every required heading. This prevents an expensive late rewrite for missing sections.
