...
function piDerivedStatusReadOnlyResult(
  ...
  return { isError: true, content: [{ type: "text" as const, text: reply.text }], details: reply.structured };
}

function piMutationNoFieldsFailure(input: Parameters<typeof noMutableFieldsReceived>[0]) {
...
function piAcceptedDecisionMutationFailure(error: unknown, outcome: "created" | "updated" | "deleted") {
  const reply = acceptedDecisionMutationFailureReply(error, outcome);
  return { isError: true, content: [{ type: "text" as const, text: reply.text }], details: reply.structured };
}

function piHandoffContractFailure(error: HandoffContractError, action: "refresh" | "preparation" = "refresh") {
...
function piContextReadActions(
  ...
  const actions = projectGuidelinesReadRequired ? ["project_guidelines_show"] : [];
...
const PLANNER_COMMAND_COMPLETIONS = [
  ...
  { value: "handoff prepare", label: "handoff prepare", description: "Audit and reconcile the active phase handoff with durable entity context" },
...
export default function planPiExtension(pi: ExtensionAPI): void {
  ...
  async function runPlanner(args: string, ctx: ExtensionContext): Promise<void> {
    ...
    if (a === "project") {
      if (!b) {
        ctx.ui.notify("project actions: discuss | language | guidelines | migrate-context", "info");
        return;
      }
      if (b === "discuss") {
      ...
      if (b === "guidelines") {
        const project = await st.loadProject();
        const nextContent = await ctx.ui.editor(
      ...
      if (b === "migrate-context") {
        const preview = await st.previewLegacyProjectContextMigration();
        const summary = legacyContextMigrationSummary(preview);
        if (!preview.hasLegacyContext) {
          ctx.ui.notify(`Project context migration preview:\n${summary}\nNo migration is needed.`, "info");
          return;
        }
        const choice = await ctx.ui.select("Apply the previewed migration? This clears only migrated legacy fields.", ["apply", "cancel"]);
        if (choice !== "apply") {
          ctx.ui.notify(`Project context migration preview (no changes applied):\n${summary}`, "info");
          return;
        }
        const result = await st.migrateLegacyProjectContext();
        await st.writeGenerated();
        ctx.ui.notify(`${result.applied ? "Legacy project context migration applied and verified." : "No legacy project context required migration."}\n${legacyContextMigrationSummary(result.preview)}`, "info");
        return;
      }
    ...
    if (a === "feature") {
      ...
      if (b === "delete") {
        ...
        const outcome = await deleteFeatureCascade(st, feature.id, { cascade });
        if (!outcome.ok) {
          ctx.ui.notify(`Feature delete failed: ${outcome.error}`, "error");
          return;
        }
        const { result } = outcome;
        ctx.ui.notify(`Feature deleted: ${result.ref}${result.phaseCount > 0 ? (result.cascade ? `; deleted ${result.phaseCount} phase(s)` : `; unlinked ${result.phaseCount} phase(s)`) : ""}`, "info");
        return;
      }
      ...
      if (b === "add") {
        ...
        await st.runBatch(() => withFeatureLock(feature.id, async () => {
          ...
          phase = {
            ...
            handoff: "", handoffUpdatedAt: "", handoffAudit: null,
            handoffReadAt: "",
            handoffHistory: [],
            statusLog: [],
            sessionInfo: [],
    ...
    if (a === "task") {
      ...
      if (b === "start") {
        ...
        if (phase) {
          const features = (await st.loadFeatures()).features;
          const feature = phase.featureId ? features.find((candidate) => candidate.id === phase.featureId) : undefined;
          ...
          if (!hasValidSessionAttestation(contextInput)) {
            await st.recordContextRead({ sessionId: plannerSessionId, phaseId: phase.id, taskId: task.id, ...(phase.featureId ? { featureId: phase.featureId } : {}), requirementIds: linkedRequirementIds });
          }
        }
      ...
      if (b === "complete") {
        ...
        const completionSummary = await ctx.ui.input("Completion summary (shipped work, verification, remaining/unverified, files/decisions)");
        if (!completionSummary?.trim() || completionSummary.trim().length < 10) {
          ctx.ui.notify("Completion cancelled — provide a durable summary of at least 10 characters.", "warning");
          return;
        }
        const now = nowISO();
        const result = await st.updateTask(phase!.id, task.id, (current) => {
    ...
    if (a === "handoff") {
      ...
      if (action === "write") {
        ctx.ui.notify("Direct handoff writes are disabled because they bypass durable task/phase/feature context synchronization. Use /planner handoff prepare, confirm the exact phase, then let the agent run handoff_prepare and handoff_write.", "warning");
        return;
      }
      if (action === "prepare") {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (receivedFields.length === 0) {
        return piMutationNoFieldsFailure({
          entity: "project",
          ref: project.name,
          operation: "update",
          mutableFields,
          retryCommand: "project_update with a concise description summary and the intended metadata",
  ...
  pi.registerTool({
    name: "project_guidelines_show",
    label: "Project Guidelines Show",
    description: "Read the canonical Project Guidelines / Coding Standard section for this project. Call this when task_start or task_switch nextActions require it, and keep the guidance in working memory while doing task work.",
    parameters: Type.Object({}),
    async execute(_id, _params, _signal, _onUpdate, ctx) {
      ...
      return {
        ...
        details: {
          projectGuidelines: refreshed.projectGuidelines,
          readRecorded: Boolean(content),
        },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    label: "Project Guidelines Update",
    ...
    parameters: Type.Object({
      content: Type.String({ description: "Full Project Guidelines markdown/text. Pass an empty string to clear the section." }),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found. Use plan_init first." }], details: {} };
      ...
      const refreshed = await st.loadProject();
      return {
        content: [{ type: "text", text: refreshed.projectGuidelines.content
  ...
  pi.registerTool({
    name: "project_context_migrate",
    label: "Project Context Migrate",
    ...
    parameters: Type.Object({
      apply: Type.Optional(Type.Boolean({ description: "False previews without mutation. True explicitly applies the previewed migration and clears only the migrated legacy fields." })),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found. Use plan_init first." }], details: {} };
      ...
      if (params.apply !== true) {
        return {
          content: [{ type: "text", text: ["Project context migration preview (no changes applied).", legacyContextMigrationSummary(preview), preview.hasLegacyContext ? "Retry with apply=true to confirm." : "No migration is needed."].join("\n") }],
          details: { applied: false, preview },
        };
      }
      const result = await st.migrateLegacyProjectContext();
      await st.writeGenerated();
      return {
        content: [{ type: "text", text: [result.applied ? "Legacy project context migration applied and verified." : "No legacy project context required migration.", legacyContextMigrationSummary(result.preview)].join("\n") }],
        details: result,
      };
    },
  });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      macroTasks: Type.Optional(Type.Array(Type.Object({
        ...
        description: Type.Optional(Type.String({ description: "Macro-task description" })),
        status: Type.Optional(Type.String({ description: "Macro-task status" })),
      }), { description: "Initial macro tasks. IDs and timestamps are planner-managed." })),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found." }], details: {} };
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      macroTasks: Type.Optional(Type.Array(Type.Object({
        ...
        title: Type.String({ description: "Macro-task title" }),
        description: Type.Optional(Type.String({ description: "Macro-task description" })),
        status: Type.String({ description: "Macro-task status" }),
      }), { description: "Replace, add, remove, or reorder macro tasks. IDs/timestamps are preserved or assigned by the planner." })),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found." }], details: {} };
  ...
  pi.registerTool({
    ...
    async execute(_id, _params, _signal, _onUpdate, ctx) {
      ...
      // P104(F005)/T422: a phase can be left in-progress with no active task
      // and nothing else said so on this cheap read — render that fact,
      // priority-ordered, the same shape planner-show renders on the MCP side.
      const openPhaseWork = listOpenPhaseWork(plan.phases, plan.features.features);
      const openPhaseWorkText = openPhaseWorkLines(openPhaseWork);
      return {
        content: [{ type: "text", text: `Plan "${plan.project.name}": ${plan.features.features.length} features, ${plan.phases.length} phases, ${plan.requirements.requirements.length} requirements. ${activeTaskEvidence}${openPhaseWorkText ? `\nIn-progress phases with work left (priority order):\n${openPhaseWorkText}` : ""}\n${projectDecisionContext.content}` }],
        details: {
          ...plan,
          activeTaskState: activeTasks.length === 0 ? "none" : activeTasks.length === 1 ? "single" : "conflict",
          activeTasks,
  ...
  pi.registerTool({
    ...
    description: "DEPRECATED AND WRITE-DISABLED — retained only for proposal compatibility. Use handoff_prepare followed by handoff_write so durable task/phase/feature context is synchronized and existing handoff content is reconciled.",
    parameters: Type.Object({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [{ type: "text", text: `⚠️ plan_write_handoff is deprecated and write-disabled because it bypasses durable context synchronization. Run handoff_prepare for ${resolved.compositeRef}, then call handoff_write with the audit token and entity updates.` }],
        details: { deprecated: true, writeDisabled: true, phaseRef: resolved.compositeRef, phaseId: resolved.phase.id },
      };
    },
  });
  ...
  pi.registerTool({
    name: "handoff_list",
    label: "Handoff List",
    description: "List phases with active entity-scoped handoffs as compact summary-only pages. Full bodies are never embedded; call handoff_show for one exact phase.",
    parameters: Type.Object({
      page: Type.Optional(Type.Integer({ minimum: 1, description: "1-based page number (default 1)." })),
      pageSize: Type.Optional(Type.Integer({ minimum: 1, maximum: 25, description: "Summaries per page (default 10, maximum 25)." })),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found." }], details: {} };
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        if (error instanceof HandoffContractError) return piHandoffContractFailure(error);
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        if (error instanceof HandoffContractError) return piHandoffContractFailure(error, "preparation");
        const message = error instanceof Error ? error.message : String(error);
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      phaseUpdate: Type.Optional(Type.Object({
        progressSummary: Type.String(),
        remainingWork: Type.String(),
        decisions: Type.Optional(Type.Array(Type.String())),
      })),
      phaseNoUpdateReason: Type.Optional(Type.String({ description: "Required when phaseUpdate is omitted." })),
      featureUpdate: Type.Optional(Type.Object({ workDone: Type.String(), workRemaining: Type.String() })),
      featureNoUpdateReason: Type.Optional(Type.String({ description: "Required when featureUpdate is omitted." })),
    }),
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const st = await requirePlan(ctx);
      if (!st) return { content: [{ type: "text", text: "No .planner/ found." }], details: {} };
      ...
      const features = (await st.loadFeatures()).features;
      const taskUpdates = [];
      for (const update of params.taskUpdates ?? []) {
        const found = findTaskByRef(phases, features, update.taskRef.trim());
        if (!found || found.phase.id !== r.phase.id) return { content: [{ type: "text", text: `❌ Task ${update.taskRef} does not belong to ${r.compositeRef}.` }], details: { error: "invalid task update" } };
        taskUpdates.push({
      ...
      try {
        const result = await st.refreshPhaseHandoff(r.phase.id, {
          ...
          contextSync: {
            taskUpdates,
            ...(params.phaseUpdate ? { phaseUpdate: {
              progressSummary: params.phaseUpdate.progressSummary,
              remainingWork: params.phaseUpdate.remainingWork,
              ...(params.phaseUpdate.decisions ? { decisions: params.phaseUpdate.decisions } : {}),
            } } : {}),
            ...(params.phaseNoUpdateReason ? { phaseNoUpdateReason: params.phaseNoUpdateReason } : {}),
            ...(params.featureUpdate ? { featureUpdate: params.featureUpdate } : {}),
            ...(params.featureNoUpdateReason ? { featureNoUpdateReason: params.featureNoUpdateReason } : {}),
          },
        });
        await st.writeGenerated();
        return {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (receivedFields.length === 0) {
        return piMutationNoFieldsFailure({
          entity: "feature",
          ref: formatFeatureRef(resolvedFeature.feature.number),
          operation: "discuss",
          mutableFields,
          retryCommand: `feature_discuss ${formatFeatureRef(resolvedFeature.feature.number)}`,
          readBackCommand: `feature_get ${formatFeatureRef(resolvedFeature.feature.number)} with full=true`,
          descriptionFallback: {
            entityId: featureId,
            inputField: "description",
      ...
      const updatedDoc = await st.updateFeatures((doc) => {
        ...
        if (params.description !== undefined) feature.description = params.description.trim();
        if (params.descriptionRef !== undefined) {
          const descriptionRef = normalizeDescriptionRef(params.descriptionRef);
          if (descriptionRef) feature.descriptionRef = descriptionRef;
          else delete feature.descriptionRef;
        }
        if (params.workDone !== undefined) feature.workDone = params.workDone.trim();
        if (params.workRemaining !== undefined) feature.workRemaining = params.workRemaining.trim();
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (params.status !== undefined) {
        return piDerivedStatusReadOnlyResult("feature", formatFeatureRef(resolvedFeature.feature.number), String(params.status), resolvedFeature.feature.status);
      }
      ...
      if (receivedFields.length === 0) {
        return piMutationNoFieldsFailure({
          ...
          operation: "update",
          mutableFields,
          retryCommand: `feature_update ${formatFeatureRef(resolvedFeature.feature.number)}`,
          readBackCommand: `feature_get ${formatFeatureRef(resolvedFeature.feature.number)} with full=true`,
          descriptionFallback: {
            entityId: featureId,
            inputField: "description",
      ...
      try {
        foundDoc = await st.updateFeatures((doc) => {
          ...
          if (params.name !== undefined) feature.name = params.name.trim();
          if (params.description !== undefined) feature.description = params.description.trim();
          if (params.descriptionRef !== undefined) {
            const descriptionRef = normalizeDescriptionRef(params.descriptionRef);
            if (descriptionRef) feature.descriptionRef = descriptionRef;
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      const resolvedFeature = resolveFeatureRefStrict(features, params.featureId);
      if (!resolvedFeature.ok) return { content: [{ type: "text", text: resolvedFeature.error }], details: {} };
      const outcome = await deleteFeatureCascade(st, resolvedFeature.feature.id, { cascade: Boolean(params.cascade) });
      if (!outcome.ok) {
        return { content: [{ type: "text", text: `Feature delete failed: ${outcome.error}` }], details: { deleted: false, errorCode: outcome.errorCode }, isError: true };
      }
      const { result } = outcome;
      return {
        content: [{ type: "text", text: `Feature deleted: ${result.ref}${result.cascade ? ` (cascade: ${result.phaseCount} phases)` : result.phaseCount > 0 ? ` (unlinked: ${result.phaseCount} phases)` : ""}` }],
        details: { deleted: true, ref: result.ref, id: result.id, cascadedPhases: result.cascade ? result.phaseCount : 0, unlinkedPhases: result.cascade ? 0 : result.phaseCount },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (params.status !== undefined) {
        return piDerivedStatusReadOnlyResult("phase", formatPhaseRef(resolvedPhase.number, featureNumberOfPhase(resolvedPhase, features)), String(params.status), resolvedPhase.status);
      }
      if (params.acceptedDecisions !== undefined) {
        return {
          content: [{ type: "text", text: ACCEPTED_DECISION_RAW_REPLACEMENT_DISABLED_MESSAGE }],
      ...
      if (receivedFields.length === 0) {
        return piMutationNoFieldsFailure({
          ...
          operation: "update",
          mutableFields,
          retryCommand: `phase_update ${formatPhaseRef(resolvedPhase.number, featureNumberOfPhase(resolvedPhase, features))}`,
          readBackCommand: `phase_get ${formatPhaseRef(resolvedPhase.number, featureNumberOfPhase(resolvedPhase, features))} with full=true`,
          descriptionFallback: {
            entityId: phaseId,
            inputField: "description",
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        return piAcceptedDecisionMutationFailure(error, "created");
      }
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        return piAcceptedDecisionMutationFailure(error, "updated");
      }
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        return piAcceptedDecisionMutationFailure(error, "deleted");
      }
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (params.status === "done" && params.status !== found.task.status) {
        return {
          content: [{ type: "text", text: "Task completion transitions require task_complete with durable completion and verification evidence." }],
          details: {},
        };
      }
      if (params.acceptedDecisions !== undefined) {
      ...
      if (receivedFields.length === 0) {
        return piMutationNoFieldsFailure({
          ...
          operation: "update",
          mutableFields,
          retryCommand: `task_update ${formatPhaseRef(found.phase.number, featureNumberOfPhase(found.phase, features))}/T${pad(found.task.number)}`,
          readBackCommand: `task_get ${formatPhaseRef(found.phase.number, featureNumberOfPhase(found.phase, features))}/T${pad(found.task.number)} with full=true`,
          descriptionFallback: {
  ...
  pi.registerTool({
    name: "task_switch",
    label: "Task Switch",
    description: "Switch from active work to a temporary task, even outside priority order. Call this tool first so valid sessionInfo context can be reused; on denial, perform only the missing/stale reads listed in nextActions and retry. If nextActions includes project_guidelines_show, read that section and keep it in working memory while doing task work. Atomically snapshot/pause the source, start the target, and push a durable LIFO return target.",
    parameters: Type.Object({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (!projectGuidelinesReady) {
        ...
        return {
          content: [{ type: "text", text: `Task switch denied: read Project Guidelines before switching task work. Current state: ${projectGuidelinesReadState}.` }],
          details: { projectGuidelinesReadState, contextEligibility, nextActions },
        };
      }
      if (!contextEligibility.eligible) {
        const nextActions = piContextReadActions(
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      const found = findTaskByRef(initialPhases, features, params.taskId.trim());
      ...
      if (selection.kind === "conflict") {
        return taskStartFailure(taskStartDenied(
          ...
          selection.reason,
          ["Pause or reconcile the active tasks.", `Retry task_start ${taskRef}.`],
          { taskId: task.id },
        ), { selection });
      }
      if (selection.kind === "active" && selection.candidate?.task.id !== task.id) {
        ...
        return taskStartFailure(taskStartDenied(
          ...
          [`Use task_switch to snapshot and pause ${activeRef}.`, `Retry task_start ${taskRef}.`],
          { taskId: task.id },
        ), { selection });
      }
      ...
      await st.writeGenerated();
      const persistedTask = (await st.loadPhase(found.phase.id)).tasks.find((candidate) => candidate.id === task.id);
      if (!startedTask || persistedTask?.status !== "in-progress") {
        return taskStartFailure(taskStartDenied(
          "PERSISTENCE_VERIFICATION_FAILED",
          `The lifecycle write could not be verified as in-progress for ${taskRef}.`,
          ["Inspect task_get for the persisted state and retry task_start only after resolving the persistence failure."],
          { taskId: task.id },
        ));
      }
      ...
      return {
        ...
        details: { ...persistedTask, ...outcome, task: persistedTask, projectContextStale: projectContextReadState === "stale", higherPriorityOpenPhase: higherPriorityPhase },
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    try {
      ...
      const startupResumeProtocol = isRecapTurn ? [
        ...
        "- After presenting: if the user says yes to resuming and no task is in-progress, your NEXT action must be task_start before any CODE edit/write. If task_start returns started=false/isError, satisfy its exact prerequisites and retry; never claim the task started until details.started=true (planner ops like handoff_write are NOT code edits).",
      ].filter(Boolean).join("\n") : "";
      ...
      const contextBlock = [
        ...
        ...(extensionRules.length > 0
          ...
          : []),
        "",
        "Project-local planner usage skill (agent-only; keep in working context and do not quote in user recaps):",
        plannerSkill.customized ? `Skill drift: ${plannerSkill.message}` : "",
        plannerSkill.content,
        "End project-local planner usage skill.",
        "",
        ...
        "",
        "Project Guidelines (mandatory when present):",
        project.projectGuidelines.content.trim() || "- (none)",
        "",
        "Global rules:",
