        ...
        <div className="grid gap-3">
          {displayedWorkTree.length > 0 ? (
            displayedWorkTree.map(({ feature, totalTasks: featureTotal, doneTasks: featureDone, allPhases, hasActiveTask }) => {
              ...
              return (
                <div key={feature.id} className={`surface-card px-4 py-3 transition-colors ${hasActiveTask ? "border-[color:var(--color-status-in-progress)]/40 bg-[color:color-mix(in_srgb,var(--color-status-in-progress)_7%,transparent)]" : ""} ${feature.status === "done" ? "opacity-70 ring-1 ring-[color:color-mix(in_srgb,var(--color-status-done)_45%,transparent)] border-[color:color-mix(in_srgb,var(--color-status-done)_40%,transparent)]" : ""} ${featureRecentlyChanged ? "ring-1 ring-[color:color-mix(in_srgb,var(--accent)_55%,transparent)] bg-[color:color-mix(in_srgb,var(--accent)_12%,transparent)]" : ""}`}>
