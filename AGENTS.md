...
- **Tag stable canonici**: dopo una pubblicazione stable riuscita da `main`, `.github/workflows/publish.yml` crea il tag annotato `vX.Y.Z` corrispondente alla versione unificata; le pubblicazioni prerelease da `next` non creano tag stable.
- **Dopo la release**: una volta mergiata la PR su `main` (pubblicazione automatica + tag stable), sincronizzare `develop`: `git switch develop && git pull && git merge origin/main && git push`.
